...
export default function PopupProfilo({ visible, onClose }) {
  ...
  // Funzione ricorsiva per appiattire i permessi
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        ...
        // 🔹 Filtra i permessi se non admin
        const filtered = ruolo === "admin"
          ? flat
          : flat.filter(p => userPermissions.includes(p.id));

        setPermissionsData(filtered);

        // Imposta il primo permesso disponibile come selezionato
        if (filtered.length > 0) {
          setSelectedPermission(filtered[0].id);
        }
      } catch (err) {
